...
**ユーザーストーリー:** 開発者として、保存前のファイルや拡張子のないファイルでも文法チェックを使用したい。そうすることで、ファイルを保存する前から日本語の品質を確認できる。

#### 受入基準

1. WHEN ユーザーが新規ファイル（Untitled）を開く THEN システムは日本語テキストを解析する
2. WHEN ユーザーが拡張子のないファイルを開く THEN システムは内容に基づいて日本語を解析する
3. WHEN ユーザーがプレーンテキストモードを選択する THEN システムは日本語テキストを解析する
4. WHEN ファイルに日本語が含まれる THEN システムは言語IDに関係なく解析を実行する
5. WHEN ユーザーが設定で特定の言語IDを除外する THEN システムはその言語IDのファイルを解析しない
