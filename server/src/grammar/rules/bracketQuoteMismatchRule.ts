 ...
 * 括弧ペアの定義
 *
 * 注意: ASCII の <>, "", '' は技術文書（Markdown）で以下の用途で頻繁に使用されるため除外：
 * - <> : 矢印記号 (->/<-/=>)、HTMLタグ、型パラメータ、比較演算子
 * - "" : 文字列リテラル、JSONキー/値、UIラベル引用
 * - '' : 文字列リテラル、属性値
 *
 * 日本語文書の括弧不一致検出に特化し、誤検知を削減する。
 */
const BRACKET_PAIRS: Map<string, string> = new Map([
  ...
  ['｛', '｝'],
  ['［', '］'],
  // ASCII括弧（丸・角・波のみ、山括弧<>は矢印誤検知のため除外）
  ['(', ')'],
  ['[', ']'],
  ['{', '}'],
  // 注: ASCII引用符 "", '' は除外（コード/JSON/UIラベルで多用されるため）
]);
