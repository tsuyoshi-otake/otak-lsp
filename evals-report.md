# Japanese Grammar Evals Report

Generated: 2025-12-28

## Summary

| Metric | Value |
|--------|-------|
| Total Categories | 71 |
| Implemented Categories | 71 (100%) |
| Total Examples | 240 |
| 期待どおりの検出数 | 240 |
| 一致率 | 100% |

## Category Results

| Category | Status | 一致 | Total | 一致率 |
|----------|--------|------|-------|-------|
| 二重助詞 | PASS | 4 | 4 | 100% |
| 助詞連続 | PASS | 3 | 3 | 100% |
| 動詞-助詞不整合 | PASS | 3 | 3 | 100% |
| 冗長な助動詞 | PASS | 3 | 3 | 100% |
| 文体混在 | PASS | 2 | 2 | 100% |
| ら抜き言葉 | PASS | 4 | 4 | 100% |
| 二重否定 | PASS | 4 | 4 | 100% |
| 同じ助詞の連続使用 | PASS | 3 | 3 | 100% |
| 接続詞連続使用 | PASS | 3 | 3 | 100% |
| 逆接「が」連続使用 | PASS | 2 | 2 | 100% |
| 全角半角アルファベット混在 | PASS | 2 | 2 | 100% |
| 弱い表現 | PASS | 3 | 3 | 100% |
| 読点過多 | PASS | 2 | 2 | 100% |
| 技術用語表記 | PASS | 6 | 6 | 100% |
| 漢字開き | PASS | 5 | 5 | 100% |
| 冗長表現 | PASS | 4 | 4 | 100% |
| 重複表現（同語反復） | PASS | 4 | 4 | 100% |
| サ変動詞 | PASS | 2 | 2 | 100% |
| 主語の欠如 | PASS | 2 | 2 | 100% |
| ねじれ文 | PASS | 2 | 2 | 100% |
| 長すぎる文 | PASS | 1 | 1 | 100% |
| 同音異義語 | PASS | 2 | 2 | 100% |
| 敬語の誤用 | PASS | 3 | 3 | 100% |
| 副詞の呼応 | PASS | 3 | 3 | 100% |
| 助詞「の」の連続 | PASS | 2 | 2 | 100% |
| 修飾語の位置 | PASS | 2 | 2 | 100% |
| 曖昧な指示語 | PASS | 2 | 2 | 100% |
| 曖昧語 | PASS | 3 | 3 | 100% |
| 「べき」用法 | PASS | 2 | 2 | 100% |
| 受身の多用 | PASS | 1 | 1 | 100% |
| 名詞の連続 | PASS | 2 | 2 | 100% |
| 接続詞の誤用 | PASS | 2 | 2 | 100% |
| 文末表現の単調さ | PASS | 2 | 2 | 100% |
| 送り仮名の揺れ | PASS | 3 | 3 | 100% |
| 表記ゆれ | PASS | 3 | 3 | 100% |
| 全角半角数字・記号混在 | PASS | 2 | 2 | 100% |
| カタカナ長音 | PASS | 2 | 2 | 100% |
| 半角カナ | PASS | 2 | 2 | 100% |
| 数字表記の混在 | PASS | 2 | 2 | 100% |
| スペースと単位 | PASS | 2 | 2 | 100% |
| 括弧・引用符の不一致 | PASS | 2 | 2 | 100% |
| 日付表記ゆれ | PASS | 2 | 2 | 100% |
| ハイフン・ダッシュ・チルダ不統一 | PASS | 2 | 2 | 100% |
| 中黒の過不足 | PASS | 2 | 2 | 100% |
| 全角記号混在 | PASS | 2 | 2 | 100% |
| 文末コロン | PASS | 3 | 3 | 100% |
| 句読点スタイルの混在 | PASS | 3 | 3 | 100% |
| 引用符スタイルの混在 | PASS | 3 | 3 | 100% |
| 箇条書き記号の混在 | PASS | 3 | 3 | 100% |
| 強調記号の混在 | PASS | 2 | 2 | 100% |
| 英語表記の大文字小文字混在 | PASS | 3 | 3 | 100% |
| 単位表記の混在 | PASS | 3 | 3 | 100% |
| 人称代名詞の混在 | PASS | 3 | 3 | 100% |
| 見出しレベルの飛び | PASS | 3 | 3 | 100% |
| テーブル列数の不一致 | PASS | 3 | 3 | 100% |
| コードブロック言語指定の欠落 | PASS | 3 | 3 | 100% |
| 「及び/並びに」使い分け | PASS | 6 | 6 | 100% |
| 「又は/若しくは」使い分け | PASS | 6 | 6 | 100% |
| 常用漢字外使用 | PASS | 40 | 40 | 100% |
| 常用漢字外使用（除外） | PASS | 9 | 9 | 100% |
| 和暦初年の統一 | PASS | 4 | 4 | 100% |
| ひらがな連続 | PASS | 1 | 1 | 100% |
| カタカナ連続 | PASS | 1 | 1 | 100% |
| 漢字連続 | PASS | 1 | 1 | 100% |
| 二点リーダ偶数 | PASS | 2 | 2 | 100% |
| ダッシュ偶数 | PASS | 2 | 2 | 100% |
| 疑問符/感嘆符後の空白 | PASS | 2 | 2 | 100% |
| 括弧内句点 | PASS | 2 | 2 | 100% |
| 括弧の入れ子深さ | PASS | 2 | 2 | 100% |
| 箇条書き句点運用 | PASS | 10 | 10 | 100% |
| 文複雑度 | PASS | 4 | 4 | 100% |

## Implemented Categories

### 二重助詞 (double-particle)

- Status: PASS
- 一致率: 100% (4/4)

### 助詞連続 (particle-sequence)

- Status: PASS
- 一致率: 100% (3/3)

### 動詞-助詞不整合 (verb-particle-mismatch)

- Status: PASS
- 一致率: 100% (3/3)

### 冗長な助動詞 (redundant-copula)

- Status: PASS
- 一致率: 100% (3/3)

### 文体混在 (style-inconsistency)

- Status: PASS
- 一致率: 100% (2/2)

### ら抜き言葉 (ra-nuki)

- Status: PASS
- 一致率: 100% (4/4)

### 二重否定 (double-negation)

- Status: PASS
- 一致率: 100% (4/4)

### 同じ助詞の連続使用 (particle-repetition)

- Status: PASS
- 一致率: 100% (3/3)

### 接続詞連続使用 (conjunction-repetition)

- Status: PASS
- 一致率: 100% (3/3)

### 逆接「が」連続使用 (adversative-ga)

- Status: PASS
- 一致率: 100% (2/2)

### 全角半角アルファベット混在 (alphabet-width)

- Status: PASS
- 一致率: 100% (2/2)

### 弱い表現 (weak-expression)

- Status: PASS
- 一致率: 100% (3/3)

### 読点過多 (comma-count)

- Status: PASS
- 一致率: 100% (2/2)

### 技術用語表記 (term-notation)

- Status: PASS
- 一致率: 100% (6/6)

### 漢字開き (kanji-opening)

- Status: PASS
- 一致率: 100% (5/5)

### 冗長表現 (redundant-expression)

- Status: PASS
- 一致率: 100% (4/4)

### 重複表現（同語反復） (tautology)

- Status: PASS
- 一致率: 100% (4/4)

### サ変動詞 (sahen-verb)

- Status: PASS
- 一致率: 100% (2/2)

### 主語の欠如 (missing-subject)

- Status: PASS
- 一致率: 100% (2/2)

### ねじれ文 (twisted-sentence)

- Status: PASS
- 一致率: 100% (2/2)

### 長すぎる文 (long-sentence)

- Status: PASS
- 一致率: 100% (1/1)

### 同音異義語 (homophone)

- Status: PASS
- 一致率: 100% (2/2)

### 敬語の誤用 (honorific-error)

- Status: PASS
- 一致率: 100% (3/3)

### 副詞の呼応 (adverb-agreement)

- Status: PASS
- 一致率: 100% (3/3)

### 助詞「の」の連続 (no-particle-chain)

- Status: PASS
- 一致率: 100% (2/2)

### 修飾語の位置 (modifier-position)

- Status: PASS
- 一致率: 100% (2/2)

### 曖昧な指示語 (ambiguous-demonstrative)

- Status: PASS
- 一致率: 100% (2/2)

### 曖昧語 (ambiguous-term)

- Status: PASS
- 一致率: 100% (3/3)

### 「べき」用法 (beki-usage)

- Status: PASS
- 一致率: 100% (2/2)

### 受身の多用 (passive-overuse)

- Status: PASS
- 一致率: 100% (1/1)

### 名詞の連続 (noun-chain)

- Status: PASS
- 一致率: 100% (2/2)

### 接続詞の誤用 (conjunction-misuse)

- Status: PASS
- 一致率: 100% (2/2)

### 文末表現の単調さ (monotonous-ending)

- Status: PASS
- 一致率: 100% (2/2)

### 送り仮名の揺れ (okurigana-variant)

- Status: PASS
- 一致率: 100% (3/3)

### 表記ゆれ (orthography-variant)

- Status: PASS
- 一致率: 100% (3/3)

### 全角半角数字・記号混在 (number-width-mix)

- Status: PASS
- 一致率: 100% (2/2)

### カタカナ長音 (katakana-chouon)

- Status: PASS
- 一致率: 100% (2/2)

### 半角カナ (halfwidth-kana)

- Status: PASS
- 一致率: 100% (2/2)

### 数字表記の混在 (numeral-style-mix)

- Status: PASS
- 一致率: 100% (2/2)

### スペースと単位 (space-around-unit)

- Status: PASS
- 一致率: 100% (2/2)

### 括弧・引用符の不一致 (bracket-quote-mismatch)

- Status: PASS
- 一致率: 100% (2/2)

### 日付表記ゆれ (date-format-variant)

- Status: PASS
- 一致率: 100% (2/2)

### ハイフン・ダッシュ・チルダ不統一 (dash-tilde-normalization)

- Status: PASS
- 一致率: 100% (2/2)

### 中黒の過不足 (nakaguro-usage)

- Status: PASS
- 一致率: 100% (2/2)

### 全角記号混在 (symbol-width-mix)

- Status: PASS
- 一致率: 100% (2/2)

### 文末コロン (sentence-ending-colon)

- Status: PASS
- 一致率: 100% (3/3)

### 句読点スタイルの混在 (punctuation-style-mix)

- Status: PASS
- 一致率: 100% (3/3)

### 引用符スタイルの混在 (quotation-style-mix)

- Status: PASS
- 一致率: 100% (3/3)

### 箇条書き記号の混在 (bullet-style-mix)

- Status: PASS
- 一致率: 100% (3/3)

### 強調記号の混在 (emphasis-style-mix)

- Status: PASS
- 一致率: 100% (2/2)

### 英語表記の大文字小文字混在 (english-case-mix)

- Status: PASS
- 一致率: 100% (3/3)

### 単位表記の混在 (unit-notation-mix)

- Status: PASS
- 一致率: 100% (3/3)

### 人称代名詞の混在 (pronoun-mix)

- Status: PASS
- 一致率: 100% (3/3)

### 見出しレベルの飛び (heading-level-skip)

- Status: PASS
- 一致率: 100% (3/3)

### テーブル列数の不一致 (table-column-mismatch)

- Status: PASS
- 一致率: 100% (3/3)

### コードブロック言語指定の欠落 (code-block-language)

- Status: PASS
- 一致率: 100% (3/3)

### 「及び/並びに」使い分け (oyobi-narabini)

- Status: PASS
- 一致率: 100% (6/6)

### 「又は/若しくは」使い分け (matawa-wakushikuwa)

- Status: PASS
- 一致率: 100% (6/6)

### 常用漢字外使用 (jouyou-kanji)

- Status: PASS
- 一致率: 100% (40/40)

### 常用漢字外使用（除外） (jouyou-kanji-exclusion)

- Status: PASS
- 一致率: 100% (9/9)

### 和暦初年の統一 (era-first-year)

- Status: PASS
- 一致率: 100% (4/4)

### ひらがな連続 (hiragana-run-length)

- Status: PASS
- 一致率: 100% (1/1)

### カタカナ連続 (katakana-run-length)

- Status: PASS
- 一致率: 100% (1/1)

### 漢字連続 (kanji-run-length)

- Status: PASS
- 一致率: 100% (1/1)

### 二点リーダ偶数 (even-leader)

- Status: PASS
- 一致率: 100% (2/2)

### ダッシュ偶数 (even-dash)

- Status: PASS
- 一致率: 100% (2/2)

### 疑問符/感嘆符後の空白 (space-after-question-exclamation)

- Status: PASS
- 一致率: 100% (2/2)

### 括弧内句点 (period-before-close-bracket)

- Status: PASS
- 一致率: 100% (2/2)

### 括弧の入れ子深さ (bracket-depth)

- Status: PASS
- 一致率: 100% (2/2)

### 箇条書き句点運用 (bullet-punctuation)

- Status: PASS
- 一致率: 100% (10/10)

### 文複雑度 (sentence-complexity)

- Status: PASS
- 一致率: 100% (4/4)

## Not Implemented Categories


---

*This report was generated by the Japanese Grammar Analyzer Evals system.*