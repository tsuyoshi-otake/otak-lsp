...
記号・数字・ASCIIのみのトークン、およびマッピングにない品詞は、ノイズを避けるためハイライト対象外（`other`）として扱います。

#### カラーテーマ

コマンドパレットで **「otak-lsp: セマンティックテーマを選択」**（`otakLsp.selectTheme`）を実行すると、配色プリセットから選べます。`default` / `pastel` / `vivid` / `monochrome` / `nature` の5種類があり、既定は `pastel` です。

テーマを変更しても、あなたが設定済みの他トークンの配色は保持されます。これは otak-lsp が `editor.semanticTokenColorCustomizations.rules` 内の自身の5品詞キーだけを更新するためです。
...
| コマンド | ID | 説明 |
|---|---|---|
| otak-lsp: ON/OFF切り替え | `otakLsp.toggle` | 文法チェックとセマンティックハイライトをまとめて有効/無効にします（ステータスバーのクリックでも実行）。 |
| otak-lsp: ステータス表示 | `otakLsp.showStatus` | 出力パネルに現在の状態・設定・アクティブファイル情報を表示します。 |
| otak-lsp: 現在のファイルを解析 | `otakLsp.analyzeCurrentFile` | アクティブなファイルの全ルール解析を即時に実行し、診断を再生成します。 |
| otak-lsp: セマンティックテーマを選択 | `otakLsp.selectTheme` | セマンティックハイライトの配色プリセットを選択します。 |
...
Language Serverは責務ごとに分離されたモジュール構成を採用しています：

| モジュール | 責務 |
|-----------|------|
| `main.ts` | エントリーポイント、コンポーネント初期化 |
| `connection.ts` | LSP接続・ハンドラ登録 |
| `configManager.ts` | 設定管理・変更通知 |
| `analysisScheduler.ts` | 解析スケジューリング（デバウンス、段階実行） |
| `documentAnalyzer.ts` | 文書解析（形態素解析、文法チェック） |
| `diagnosticsPublisher.ts` | 診断結果の発行 |
